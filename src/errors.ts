...
export const authenticationErrorCodec = t.partial({message: t.string}, 'authenticationErrorCodec');
...
export const aiArenaExceptionCodec = t.union([
  ...
  t.intersection([t.type({type: t.literal(ErrorType.AUTHENTICATION_ERROR)}), authenticationErrorCodec]),
  t.intersection([t.type({type: t.literal(ErrorType.AUTHORIZATION_ERROR)}), authorizationErrorCodec]),
]);
...
export class AuthenticationError extends AiArenaException {
  constructor(data: t.TypeOf<typeof authenticationErrorCodec>) {
    super({type: ErrorType.AUTHENTICATION_ERROR, message: 'Not authenticated', ...data});
  }
}

export class AuthorizationError extends AiArenaException {
  constructor(data: t.TypeOf<typeof authorizationErrorCodec>) {
    super({type: ErrorType.AUTHORIZATION_ERROR, message: 'Not authorized', ...data});
  }
}
