...
export class UserResolver {
  constructor(
    ...
    if (!this.isContextWithExecutor(context)) {
      throw new AssertException({message: 'UserResolver: unhandled context structure', values: {context}});
    }
  ...
  async profile(): Promise<User> {
    ...
    return notNull(await this.userRepository.findOne(this.executor, {where: {id: this.executor.id}}));
  }
}
